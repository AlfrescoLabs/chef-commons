...
action :run do
  ...
  begin
    ...
    ssl.each do |ssl_item_name, ssl_item_value|
      ...
      ssl_file = "#{ssl_folder}/#{ssl_filename}.#{ssl_item_name}"
      file ssl_file do
        action :create
    ...
    ssl_key_file = "#{ssl_folder}/#{ssl_filename}.key"
    ssl_crt_file = "#{ssl_folder}/#{ssl_filename}.crt"
    ssl_chain_file = "#{ssl_folder}/#{ssl_filename}.chain"
    ssl_nginxcrt_file = "#{ssl_folder}/#{ssl_filename}.nginxcrt"
    ssl_dhparam_file = "#{ssl_folder}/#{ssl_filename}.dhparam"
    unless skip_certificate_creation
